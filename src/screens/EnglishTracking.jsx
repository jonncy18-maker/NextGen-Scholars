...
function findActivePeriod(periods) {
  ...
  const t = today();
  return (
    periods.find((p) => p.start_date <= t && p.end_date >= t) ??
            ...
            <div key={cat} className="et-cat-bar-row">
              ...
              <div className="et-cat-bar-tracks">
                <div className="et-cat-bar-track et-cat-bar-track--act">
                  <div
                    className="et-cat-bar-fill et-cat-bar-fill--act"
            ...
            <div className="et-cat-bar-tracks">
              {expected != null && (
                <div className="et-cat-bar-track et-cat-bar-track--exp">
                  <div
                    className="et-cat-bar-fill et-cat-bar-fill--exp"
              ...
              <div className="et-cat-bar-track et-cat-bar-track--act">
                <div
                  className={`et-cat-bar-fill et-cat-bar-fill--act et-cat-bar-fill--${status}`}
                  style={{ width: `${actWidth}%` }}
                />
    ...
    return (
      <div className="et-row et-row--editing">
        ...
        <div className="et-row-edit-actions">
          <button className="et-edit-save" onClick={handleSave} disabled={saving}>
            {saving ? '…' : 'Save'}
          </button>
          <button className="et-edit-cancel" onClick={() => setEditing(false)}>
            Cancel
          </button>
        </div>
      </div>
  ...
  return (
    <div className="et-row">
      ...
      <div className="et-row-right">
        ...
        <button className="et-row-edit" onClick={startEdit} title="Edit">
          ✎
        </button>
        <button className="et-row-del" onClick={handleDelete} disabled={deleting} title="Delete">
          ×
        </button>
      </div>
    </div>
  ...
  return (
    ...
    >
      <section>
          ...
          {sessions === null ? (
            ...
            <>
              <div className="et-progress-top-row">
                ...
                {expectedNow != null && <StatusChip status={overallStatus} />}
              </div>

              {sessionTypeName && <div className="et-session-type-label">{sessionTypeName}</div>}
        ...
        {/* Category bar chart */}
        {sessions !== null && period && <CategoryBarChart period={period} sessions={sessions} />}

        {/* 8-week chart */}
        {sessions !== null && sessions.length > 0 && <WeeklyChart sessions={sessions} />}
        ...
        {showForm && form && (
          <form className="et-form" onSubmit={handleSubmit}>
            ...
            <div className="et-field">
              <label>
                Notes <span className="et-optional">(optional)</span>
              </label>
              <textarea
                rows={2}
          ...
          {Object.entries(grouped).map(([cat, rows]) => (
            <div key={cat} className="et-cat-group">
              <button className="et-cat-group-toggle" onClick={() => toggleCat(cat)}>
                ...
                <span className="et-cat-group-count">
                  {rows.length} session{rows.length !== 1 ? 's' : ''}
                </span>
                <span className="et-cat-group-hrs">
                  {rows.reduce((s, r) => s + (r.duration_minutes || 0) / 60, 0).toFixed(1)}h
                </span>
                <span className="et-cat-group-chevron">{collapsedCats?.has(cat) ? '▶' : '▼'}</span>
              </button>
