          ...
          return (
            <div key={sub.id} className={`ef-pending-item${isRejected ? ' is-rejected' : ''}`}>
              <div className="ef-pending-item-main">
                ...
                <span className="ef-pending-item-amt">
                ...
                <div className="ef-resubmit-form-wrap">
                  <div className="ef-resubmit-form">
                    ...
                    <label className="ef-edit-field">
                      ...
                      <select
                        value={editDraft.avb}
                        onChange={(e) => setEditDraft((d) => ({ ...d, avb: e.target.value }))}
                      >
                        {AVB_OPTIONS.map((o) => (
                    ...
                    <div className="ef-edit-actions">
                      <button
                        className="ef-edit-save"
                        onClick={() =>
                          editMode === 'edit' ? onHandleEditPending(sub) : onHandleResubmit(sub)
                        }
                      >
                        {editMode === 'edit' ? 'Save changes' : 'Resubmit'}
                      </button>
                      <button className="ef-edit-cancel" onClick={onCancelDraft}>
                        Cancel
                      </button>
                    </div>
                  </div>
                  <div className="ef-ai-chat">
                    {aiLog.length > 0 && (
                      <div className="ef-ai-chat-log">
                        {aiLog.map((m, i) => (
                          <div key={i} className={`ef-ai-chat-msg ef-ai-chat-${m.role}`}>
                            {m.text}
                          </div>
                        ))}
                      </div>
...
function ApprovedExpensesTable({
  currentSem,
  semExpenses,
  groupBy,
  setGroupBy,
  ...
  function renderEntryRow(e, i) {
      ...
      return (
        <React.Fragment key={e.id || i}>
          <tr className="ef-entries-editing-hd">
            ...
            <td>
              <span className="ef-entries-cat" data-cat={e.cat}>
                {e.cat}
              </span>
            </td>
            <td className="ef-entries-date">{e.date}</td>
            <td className="ef-entries-right ef-entries-amount">
              ₱{Math.round(total).toLocaleString('en-US')}
            </td>
            <td>
            ...
            <td className="ef-entries-actions">
              <button className="ef-row-cancel" onClick={onCancelEdit}>
                Cancel
              </button>
            </td>
          </tr>
            ...
            <td colSpan={6}>
              <div className="ef-edit-form">
                <label className="ef-edit-field">
                  ...
                  <input
                    value={editDraft.item}
                    onChange={(ev) => setEditDraft((d) => ({ ...d, item: ev.target.value }))}
                  />
                </label>
                <label className="ef-edit-field">
                  <span>Category</span>
                  <select
                    value={editDraft.cat}
                    onChange={(ev) => setEditDraft((d) => ({ ...d, cat: ev.target.value }))}
                  >
                    {EXPENSE_CATS.map((c) => (
                      <option key={c}>{c}</option>
                    ))}
                ...
                <label className="ef-edit-field">
                  ...
                  <input
                    ...
                    min="1"
                    value={editDraft.qty}
                    onChange={(ev) => setEditDraft((d) => ({ ...d, qty: ev.target.value }))}
                  />
                </label>
                <label className="ef-edit-field">
                  <span>Date</span>
                ...
                <label className="ef-edit-field">
                  ...
                  <select
                    value={editDraft.avb}
                    onChange={(ev) => setEditDraft((d) => ({ ...d, avb: ev.target.value }))}
                  >
                    {AVB_OPTIONS.map((o) => (
                      <option key={o}>{o}</option>
                    ))}
                ...
                <label className="ef-edit-field">
                  ...
                  <input
                    value={editDraft.vendor}
                    onChange={(ev) => setEditDraft((d) => ({ ...d, vendor: ev.target.value }))}
                  />
                </label>
                <div className="ef-edit-actions">
                  <button className="ef-edit-save" onClick={() => onSaveEdit(e)}>
                    Save changes
                  </button>
                  <button className="ef-edit-cancel" onClick={onCancelEdit}>
                    Cancel
                  </button>
                </div>
              </div>
    ...
    return (
      <tr
        key={i}
        className={[
          e.avb !== AVB_OPTIONS[0] ? 'ef-entries-budget' : '',
      ...
      >
        ...
        <td className="ef-entries-actions">
          {!isPending && (
            <>
              <button className="ef-row-edit" onClick={() => onStartEdit(e)}>
                Edit
              </button>
              <button className="ef-row-del" onClick={() => onRequestDelete(e)}>
                Delete
              </button>
            </>
          )}
  ...
  return (
    <div className="ef-entries">
      <div className="ef-entries-header">
        <span className="ef-entries-title">
          {currentSem} · {semExpenses.length} item{semExpenses.length !== 1 ? 's' : ''}
        </span>
                  ...
                  <tbody>
                    <tr className="ef-group-hd" onClick={() => toggleGroup(group.key)}>
                      <td colSpan={6}>
                        ...
                        <span className="ef-group-meta">
                          {group.rows.length} item{group.rows.length !== 1 ? 's' : ''}
                        </span>
                        <span className="ef-group-total">
                  ...
                  {!collapsed && (
                    <tbody>
                      ...
                      <tr className="ef-subtotal">
                        <td colSpan={3} className="ef-subtotal-label">
                          Subtotal — {group.label}
                        </td>
                        <td className="ef-subtotal-amt ef-entries-right">
                          ₱{Math.round(group.total).toLocaleString('en-US')}
                        </td>
                        <td />
                        <td />
...
function ExpenseForm({ scholar, onLogout }) {
  ...
  const [multiRows, setMultiRows] = useState(() => [
    makeEmptyRow(scholar.defaultSem),
    makeEmptyRow(scholar.defaultSem),
  ]);
  ...
  const filledRows = multiRows.filter(
    (r) => r.item.trim() && r.amount && !isNaN(parseAmount(r.amount)) && parseAmount(r.amount) > 0
  );
  ...
  function saveEdit(originalExp) {
    ...
    setExpensesBySem((prev) => {
      ...
      return {
        ...prev,
        [sem]: (prev[sem] || []).map((e) => (e.id === originalExp.id ? updated : e)),
      };
    });
    updateExpense(originalExp.id, fields).catch((err) =>
  ...
  async function handleEditPending(sub) {
    ...
    try {
      ...
      setPendingSubmissions((prev) =>
        prev.map((s) => (s.id === sub.id ? updated || { ...s, expense_data: expData } : s))
      );
      cancelResubmit();
    } catch (err) {
      console.error('edit pending failed:', err);
    }
  }
  ...
  async function handleAiEdit() {
    ...
    try {
      ...
      if (c) {
        ...
        setAiEditLog((prev) => [
          ...prev,
          { role: 'ai', text: 'Done — updated the fields above. Review, then save.' },
        ]);
      } else {
        setAiEditLog((prev) => [
          ...prev,
  ...
  const actualTotal = semExpenses
    .filter((e) => e.avb === AVB_OPTIONS[0])
    .reduce((t, e) => t + (e.amount || 0) * (e.qty || 1), 0);
  const budgetTotal = semExpenses
    .filter((e) => e.avb !== AVB_OPTIONS[0])
    .reduce((t, e) => t + (e.amount || 0) * (e.qty || 1), 0);
        ...
        <main className="ef-main">
          {expensesBySem !== null && (
            <div className="ef-summary">
              <div className="ef-summary-stat">
                ...
                <strong className="ef-summary-val">
                  ₱{Math.round(actualTotal).toLocaleString('en-US')}
                </strong>
              </div>
              {budgetTotal > 0 && (
                <div className="ef-summary-stat">
                  ...
                  <strong className="ef-summary-val">
                    ₱{Math.round(budgetTotal).toLocaleString('en-US')}
                  </strong>
                </div>
              )}
              <div className="ef-summary-stat">
          ...
          {entryMode === 'single' && (
            <div className="ef-entry-card">
              <div className="ef-entry-card-hd">
                ...
                <button className="ef-entry-card-close" onClick={() => setEntryMode(null)}>
                  ✕
                </button>
              </div>
              ...
              <form className="ef-grid" onSubmit={handleSingleSubmit}>
                ...
                <div className="ef-field ef-field-wide">
                  <label>
                    Vendor <span className="ef-optional">(optional)</span>
                  </label>
                  <input
                    type="text"
                ...
                <div className="ef-submit-row">
                  <button
                    type="submit"
                    disabled={!singleValid || submitState === 'submitting'}
          ...
          {entryMode === 'multiple' && (
            <div
              ...
              onClick={(e) => {
                if (e.target === e.currentTarget) setEntryMode(null);
              }}
            >
              <div className="ef-modal-panel">
                ...
                {multiSubmitState === 'done' && (
                  <div className="ef-toast ef-modal-toast">
                    ✓ {filledRows.length} expense{filledRows.length !== 1 ? 's' : ''} submitted for
                    review
                  </div>
                )}
                <form onSubmit={handleMultiSubmit} className="ef-modal-form">
                      ...
                      <tbody>
                        {multiRows.map((row, idx) => (
                          <tr key={row._id} className="ef-multi-row">
                            <td>
                              <input
                                className="ef-multi-input ef-multi-wide"
                                type="text"
                                placeholder="Item name"
                                value={row.item}
                                onChange={(e) => updateMultiRow(idx, 'item', e.target.value)}
                              />
                            </td>
                            <td>
                              <select
                                className="ef-multi-input ef-multi-cat"
                                value={row.cat}
                                onChange={(e) => updateMultiRow(idx, 'cat', e.target.value)}
                              >
                                {EXPENSE_CATS.map((c) => (
                                  <option key={c}>{c}</option>
                                ))}
                            ...
                            <td>
                              <input
                                className="ef-multi-input ef-multi-num"
                                type="text"
                                inputMode="decimal"
                                placeholder="0.00"
                                value={row.amount}
                                onChange={(e) => updateMultiRow(idx, 'amount', e.target.value)}
                              />
                            </td>
                            <td>
                              <input
                                className="ef-multi-input ef-multi-sm"
                                type="number"
                                min="1"
                                value={row.qty}
                                onChange={(e) => updateMultiRow(idx, 'qty', e.target.value)}
                              />
                            </td>
                            <td>
                              <input
                                className="ef-multi-input ef-multi-date"
                                type="date"
                                value={row.date}
                            ...
                            <td>
                              <select
                                className="ef-multi-input ef-multi-avb"
                                value={row.avb}
                                onChange={(e) => updateMultiRow(idx, 'avb', e.target.value)}
                              >
                                {AVB_OPTIONS.map((o) => (
                                  <option key={o}>{o}</option>
                                ))}
                            ...
                            <td>
                              <input
                                ...
                                placeholder="Optional"
                                value={row.vendor}
                                onChange={(e) => updateMultiRow(idx, 'vendor', e.target.value)}
                              />
                            </td>
                            <td>
                              <button
                                type="button"
                                className="ef-multi-remove"
                  ...
                  <div className="ef-multi-footer ef-modal-footer">
                    <button
                      type="button"
                      className="ef-multi-add-row"
                      onClick={() =>
                        setMultiRows((prev) => [...prev, makeEmptyRow(scholar.defaultSem)])
                      }
                    >
                      + Add another expense
                    </button>
                    ...
                    >
                      {multiSubmitState === 'done'
                        ? '✓ Submitted'
                        : multiSubmitState === 'submitting'
                          ? 'Submitting…'
                          : `Submit ${filledRows.length > 0 ? filledRows.length + ' ' : ''}expense${filledRows.length !== 1 ? 's' : ''}`}
                    </button>
                  </div>
                </form>
              </div>
