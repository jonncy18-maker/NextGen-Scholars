...
| `app/api/mcp/route.js`, `lib/mcp-auth.js`, `lib/mcp/json-schema.js` | MCP server exposing the full `lib/ai/tools.js` registry to any MCP client (Claude Desktop/Code, etc.) with full mentor access, authenticated by a static personal API key (`mcp_api_keys` table) rather than a Better Auth session. Unlike `/api/agent`, mutating tools run immediately on `tools/call` — the human-in-the-loop is the MCP client's own tool-approval step, not a confirm-card round trip. See `docs/MCP.md`. |
