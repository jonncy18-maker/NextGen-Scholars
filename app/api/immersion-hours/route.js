...
// GET — mentor-only. Returns { [scholarKey]: { currentHours, targetHours,
// hoursThisWeek, expectedHours, lastSessionAt, status } } for every scholar
// with an Immersion account. Read-only: the `ngs_scholars_reader` Postgres
// role backing IMMERSION_DATABASE_URL can only SELECT from scholar_pace,
// user_total_hours, and users in that project — nothing else, no writes.
export const GET = withErrorHandling(async (request) => {
  await requireMentor(request);

  const ids = Object.values(IMMERSION_USER_ID);
  ...
  const result = {};
  for (const [scholarKey, userId] of Object.entries(IMMERSION_USER_ID)) {
