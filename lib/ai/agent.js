  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    if (writes.length) {
      ...
      for (const c of writes) {
        ...
        proposals.push({ name: c.name, args: c.input || {}, summary: describeCall(c.name, c.input) });
      }
      const note = content.filter((b) => b.type === 'text').map((b) => b.text).join('\n').trim();
    ...
    for (const c of calls) {
      ...
      toolResults.push({ type: 'tool_result', tool_use_id: c.id, content: JSON.stringify(payload) });
    }
