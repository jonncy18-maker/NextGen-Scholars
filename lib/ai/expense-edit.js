...
//
// Read-only on the DB: this only asks Claude to rewrite the provided rows. The
...
- Your entire response must be a valid JSON array starting with [ and ending with ].`;
  ...
  //
  // The `cat` fallback below rewrites anything off the allowed list to 'Other',
  // and ExpenseWorkbench.jsx's diffRow() then rebuckets from CAT_TO_BUCKET —
  // so EXPENSE_CATS must be the single shared list from src/constants.js, never
  // a local copy. A stale copy here (missing the travel/milestone categories)
  // silently recategorized every travel expense the mentor edited as
  // Other/college, moving money between the bucket totals that the public
  // profile pages publish.
  ...
  return { rows: rowsOut, model: CLAUDE_MODEL };
}
