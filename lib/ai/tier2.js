...
// question plus a compact scholar context bundle and returns a structured
// answer. Read-only: the model never writes to the database.
//
...
export async function tier2Ask(question, ctx, apiKey, history, provider = 'gemini') {
    ...
    if (history && history.length > 0) {
      history.forEach((m, i) => {
        messages.push({
          ...
          text: i === 0 ? ctxBlock + m.text : m.text,
        });
      });
      messages.push({ role: 'user', text: question });
    } else {
      messages.push({ role: 'user', text: `${ctxBlock}Question: ${question}` });
    }
    const res = await callClaude({
      apiKey,
      system: NGS_SYSTEM_PROMPT,
      messages: messages.map((m) => ({ role: m.role, content: m.text })),
      // Same fix as expense-edit.js/budget.js: thinking tokens count against
      // maxTokens on claude-sonnet-5 — 'low' effort keeps this bounded Q&A
      // task cheap and leaves the budget for the actual answer.
      maxTokens: 1024,
      effort: 'low',
    });
    if (!res.ok) return { answered: false, error: res.error };
    const text = textFromMessage(res.message);
    if (!text) return { answered: false, error: 'Claude returned an empty response.' };
    return { answered: true, answer: text, model: CLAUDE_MODEL };
  }
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (inner)
        message = res.status === 429 ? `Gemini quota exceeded — ${inner.split('.')[0]}.` : inner;
    } catch {
...
// Builds one call from every scholar's context bundle and returns a single
...
export async function tier2WeeklyReport(contexts, apiKey, provider = 'gemini') {
  ...
  if (provider === 'claude') {
    const res = await callClaude({
      ...
      // Same fix as expense-edit.js/budget.js: thinking tokens count against
      // maxTokens on claude-sonnet-5. The report follows a fixed template
      // (bounded per-scholar bullets, not open-ended writing), so 'low'
      // effort avoids a truncated reasoning pass eating the whole budget;
      // the higher ceiling covers a full cohort of per-scholar sections.
      maxTokens: 4096,
      effort: 'low',
    });
    if (!res.ok) return { answered: false, error: res.error };
