...
// Same contract as action.js and expense-edit.js: Gemini only PROPOSES a list
...
// open-ended questions and all write intents reach Gemini.
...
const OPS = ['create_category', 'update_category', 'archive_category', 'set_plan'];

function php(n) {
  return '₱' + Math.round(Number(n) || 0).toLocaleString('en-US');
}

// A negative total cost yields a negative monthly accrual, which renders as a
// nonsense "save -₱400/month". planned_php was already clamped; these weren't.
function clampNonNegative(n) {
  return n === null ? null : Math.max(0, n);
}

function monthlyAccrual(cat) {
  const t = Number(cat.sinking_target_php);
  const m = Number(cat.sinking_months);
  if (!t || !m || m <= 0) return 0;
  return t / m;
}
...
const WRITE_VERBS = /\b(set|add|change|rename|remove|delete|drop|make|create|put|fill|raise|lower|increase|decrease|update|bump)\b/i;
const QUESTION_START = /^(what|what's|whats|how|which|when|why|who|is|are|do|does|did|can|could|should|show|list|tell)\b/i;
...
export function resolveBudgetRead(text, { categories, plan, month }) {
  ...
  // "what am I saving for" / sinking funds
  if (asks('saving for', 'sinking', 'set aside', 'putting aside')) {
    const sinking = active.filter(c => c.kind === 'sinking');
    if (sinking.length === 0) {
      return 'No sinking funds set up yet — those are for costs that don\'t arrive every month, like registration or insurance.';
    }
    const lines = sinking.map(c => {
      const acc = monthlyAccrual(c);
      return acc > 0
        ? `• ${c.name} — ${php(c.sinking_target_php)} over ${c.sinking_months} months, about ${php(acc)}/month`
        : `• ${c.name} — no total cost set yet, so there's no monthly amount to save`;
    });
    return `You're saving toward:\n${lines.join('\n')}`;
  }
  ...
  if (asks('what categories', 'list my categories', 'which categories', 'what am i budgeting')) {
    ...
    const lines = active.map(c => `• ${c.name} — ${amountFor(c) ? php(amountFor(c)) : 'not set'}`);
  ...
  if (named && asks('how much', 'what is', "what's", 'budgeted for', 'set for')) {
    ...
    const extra = named.kind === 'sinking' && monthlyAccrual(named) > 0
      ? ` (a sinking fund — ${php(named.sinking_target_php)} over ${named.sinking_months} months works out to about ${php(monthlyAccrual(named))}/month)`
      : '';
    return amt
...
function stateForPrompt({ categories, plan, month }) {
  return {
    month,
    categories: categories.filter(c => !c.archived_at).map(c => ({
      id: String(c.id),
      name: c.name,
      kind: c.kind,
      rollup: c.rollup,
      sinking_target_php: c.sinking_target_php == null ? null : Number(c.sinking_target_php),
      sinking_months: c.sinking_months == null ? null : Number(c.sinking_months),
      planned_php: Number(plan[c.id]) || 0,
    })),
    archived: categories.filter(c => c.archived_at).map(c => ({ id: String(c.id), name: c.name })),
  };
}
...
- sinking: NOT billed monthly — she saves toward it (annual registration, insurance, tires). A sinking category needs sinking_target_php (the whole cost) and sinking_months (how many months until it is due). The monthly amount is target divided by months.
...
Valid ops:
{"op":"create_category","name":"...","kind":"...","rollup":"...","sinking_target_php":null,"sinking_months":null,"planned_php":0}
{"op":"update_category","id":"...","name":"...","kind":"...","rollup":"...","sinking_target_php":null,"sinking_months":null}
{"op":"archive_category","id":"..."}
{"op":"set_plan","category_id":"...","planned_php":0}
...
- If she gives a yearly cost for something irregular, make it a sinking category and set sinking_months to 12 rather than putting the whole amount in one month.
...
Her request: ${text}`;
  ...
  try {
    res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
      method: 'POST',
      // Bypass Next's Data Cache — a POST fetch from a route handler is
      // otherwise cached by url+body (see CLAUDE.md, Neon/Data Cache note).
      cache: 'no-store',
    ...
    return { error: `Could not reach the AI service: ${err.message}` };
  }

  const gJson = await res.json().catch(() => null);
...
export function sanitizeOps(ops, { categories }) {
  ...
  return (Array.isArray(ops) ? ops : []).reduce((out, o) => {
    ...
    if (o.op === 'create_category') {
      ...
      if (!name) return out;
      const months = num(o.sinking_months);
      out.push({
        op: 'create_category',
        name,
        ...
        sinking_months: months === null ? null : Math.max(1, Math.round(months)),
    ...
    if (o.op === 'update_category') {
      ...
      if ('name' in o) {
        const n = String(o.name ?? '').trim().slice(0, 60);
        if (n) patch.name = n;
      }
      if (KINDS.includes(o.kind)) patch.kind = o.kind;
      if (ROLLUPS.includes(o.rollup)) patch.rollup = o.rollup;
      if ('sinking_target_php' in o) patch.sinking_target_php = clampNonNegative(num(o.sinking_target_php));
      if ('sinking_months' in o) {
        const m = num(o.sinking_months);
        patch.sinking_months = m === null ? null : Math.max(1, Math.round(m));
      }
    ...
    if (!known.has(id)) return out;
    out.push({ op: 'set_plan', category_id: id, planned_php: Math.max(0, num(o.planned_php) ?? 0) });
    return out;
  }, []);
}
