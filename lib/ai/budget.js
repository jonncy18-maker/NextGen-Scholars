...
// Pulls the outermost {...} out of the model's raw text rather than
// requiring the ENTIRE trimmed string to already be valid JSON. A stray
// lead-in sentence or a trailing aside survives this; a markdown fence
// around the object does too, since the fence characters just fall outside
// the first "{" / last "}" span. Still fails (correctly) on output that's
// genuinely truncated mid-object — that's a real error, not one to paper
// over by best-effort-repairing broken JSON.
...
export async function resolveBudgetOps(text, state, apiKey) {
  ...
  const prompt = `Current budget state:
...
Her request: ${text}`;
  ...
  const res = await callClaude({
    ...
    // 1500 was too tight once thinking counted against it (see claude.js) —
    // a truncated reasoning pass ate the whole budget and left no room for
    // the JSON itself, which is what "not valid JSON" actually was. 'low'
    // effort keeps this bounded, short-answer task cheap and fast; the
    // larger ceiling is headroom, not an invitation to reason longer.
    maxTokens: 4096,
    effort: 'low',
  });
  ...
  try {
    ...
    // Without this, a parse failure was a dead end — no way to tell a
    // genuinely malformed response from a truncated one after the fact.
    // Vercel captures console.error from serverless functions, so this is
    // recoverable from the logs instead of guessed at from a screenshot.
    console.error('[ask-budget] Could not parse Claude response as JSON:', raw.slice(0, 1000));
