...
async function callClaudeJson({ apiKey, system, userText, file, maxTokens }) {
  ...
  const res = await callClaude({
    ...
    // maxTokens on claude-sonnet-5, and a truncated reasoning pass here cuts
    // off the extracted JSON mid-object. 'low' effort keeps this bounded
    // extraction task cheap and leaves the budget for the actual output.
    maxTokens,
    effort: 'low',
  });
...
export async function tier3Ingest(options, _scholar, apiKey, provider = 'gemini') {
...
Output: [{"item":"Tuition","amount":3500,"qty":1,"cat":"Tuition","date":"${today}","vendor":""},{"item":"Nursing textbook","amount":425,"qty":2,"cat":"Books","date":"${today}","vendor":""}]`;
  ...
  if (!Array.isArray(result.parsed)) {
    return {
      answered: false,
      error: `Could not parse ${provider === 'claude' ? 'Claude' : 'Gemini'} response as JSON: ${result.rawText.slice(0, 300)}`,
    };
  }
...
export async function tier3GradeIngest(options, _scholar, apiKey, provider = 'gemini') {
...
export async function tier3EnglishIngest(text, categories, apiKey, provider = 'gemini') {
  const today = new Date().toISOString().slice(0, 10);
