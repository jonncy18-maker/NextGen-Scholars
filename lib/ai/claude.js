...
//
// `effort` (optional: 'low' | 'medium' | 'high' | 'xhigh' | 'max') maps to
// output_config.effort. claude-sonnet-5 runs extended thinking ON BY
// DEFAULT — unlike older models, omitting `thinking` does not mean no
// reasoning happens. Thinking tokens count against `maxTokens`, so a caller
// with a tight token budget doing bounded, non-creative work (classify,
// extract, propose a short op list) should pass 'low' to keep reasoning
// spend small and leave headroom for the actual answer — otherwise a low
// maxTokens can get eaten by reasoning before any visible text is written,
// which truncates the JSON output mid-object.
export async function callClaude({ apiKey, system, messages, tools, maxTokens = 2048, effort }) {
