...
//
// `cache: 'no-store'` is load-bearing, not defensive: the driver issues each
// query as a POST through global fetch, which Next.js patches with its Data
// Cache. In Next 14 route handlers `dynamic = 'force-dynamic'` does NOT opt
// those fetches out (it never sets the store's revalidate to 0, which the
// POST-method escape hatch requires), so identical query bodies — e.g.
// bootstrap's `select * from expenses` — get cached on Vercel with a one-year
// TTL and served as a frozen snapshot across requests AND deploys. Without
// this option, reads go stale the moment any cached query's data changes.
let _client = null;
function client() {
  if (!_client) {
