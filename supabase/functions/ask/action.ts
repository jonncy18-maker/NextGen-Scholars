...
function gcFee(n: number): number { return Math.ceil(n / 500) * 15 }

export interface ActionResult {
  action?: 'record_send'
  items?:  { id: string; item: string; amount: number; sem: string }[]
  fee?:    number
  total?:  number
  note?:   string
  model?:  string
...
export async function resolveSendAction(
  ...
  const { data: rows, error } = await sb
    .from('expenses')
    .select('id,item,amount,sem,cat,sent')
    .eq('scholar', scholar)
    .neq('sent', 'Yes')
...
- NEVER invent ids. Use only ids that appear in the list above.`
  ...
  try {
    ...
    return { error: `Gemini network error: ${(err as Error).message}` }
  }
  ...
  const out = json?.candidates?.[0]?.content?.parts?.[0]?.text as string | undefined
